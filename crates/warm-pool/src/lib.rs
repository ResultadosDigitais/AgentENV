        ...
        loop {
            if !has_immediate_work {
                ...
                loop {
                    ...
                    // Wait until the earliest of: a scheduled wake (backoff
                    // retries via `request_maintenance_after`), the idle TTL
                    // expiry (so the fill target can decay and excess idle
                    // resources drain), or an explicit signal.
                    let now = Instant::now();
                    if let Some(wake_at) = signal.wake_at {
                        if wake_at <= now {
                    ...
                    let wake_in = signal
                        .wake_at
                        .map(|wake_at| wake_at.saturating_duration_since(now));
                    let idle_remaining = self.idle_ttl_remaining();
                    match (wake_in, idle_remaining) {
                        (Some(wake_in), Some(idle_remaining)) => {
                            let wait = wake_in.min(idle_remaining);
                            let (guard, timeout) =
                                self.maintenance_cv.wait_timeout(signal, wait).unwrap();
                            signal = guard;
                            if timeout.timed_out() && !signal.stop && !signal.pending {
                                let now = Instant::now();
                                if signal.wake_at.is_some_and(|wake_at| wake_at <= now) {
                                    // The scheduled wake fired first.
                                    signal.wake_at = None;
                                    break;
                                }
                                // The idle TTL expired: run a cycle so the
                                // fill target can decay.
                                signal.pending = true;
                            }
                        }
                        (Some(wake_in), None) => {
                            let (guard, _) =
                                self.maintenance_cv.wait_timeout(signal, wake_in).unwrap();
                            signal = guard;
                        }
                        (None, Some(idle_remaining)) => {
                            let (new_signal, timeout) = self
                                .maintenance_cv
                                .wait_timeout(signal, idle_remaining)
                                .unwrap();
                            signal = new_signal;
                            if timeout.timed_out() && !signal.stop && !signal.pending {
                                signal.pending = true;
                ...
                // This cycle also consumes any already-due scheduled wake:
                // an expired instant left behind would survive to a later
                // `request_maintenance_after`, whose earliest-wins `min`
                // would then fire an immediate extra cycle and defeat the
                // new backoff.
                if signal.wake_at.is_some_and(|wake_at| wake_at <= Instant::now()) {
                    signal.wake_at = None;
                }
            }
...
mod tests {
    ...
    #[test]
    fn pending_cycle_clears_expired_scheduled_wake() {
        let pool: &'static WarmPool<u32> = Box::leak(Box::new(WarmPool::new(PoolConfig {
            low_watermark: 0,
            high_watermark: 10,
            maintenance_enabled: true,
